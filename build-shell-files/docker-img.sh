...
if cd java-vm-deploy && test -f Dockerfile; then
    ...
    if  cd .. && docker build -f build-pipeline/default-build-files/Dockerfile -t gcr.io/terraform-265913/java-app . && docker push gcr.io/terraform-265913/java-app;then
        echo "Docker image pushed"
    else
        echo "Error at building docker image" && exit 1
    fi
